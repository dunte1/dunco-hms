    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
                ...
                <form method="POST" action="{{ route('hms.ot.store') }}" class="p-6 space-y-6">
                    @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="procedure_name" value="{{ old('procedure_name') }}" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500" placeholder="e.g., Appendectomy, Knee Replacement">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="procedure_description" rows="2" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500" placeholder="Detailed description of the procedure...">{{ old('procedure_description') }}</textarea>
                            </div>
                            <div>
                                ...
                                <select name="procedure_type" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500">
                                    <option value="elective" {{ old('procedure_type') == 'elective' ? 'selected' : '' }}>Elective</option>
                                    <option value="emergency" {{ old('procedure_type') == 'emergency' ? 'selected' : '' }}>Emergency</option>
                                    <option value="urgent" {{ old('procedure_type') == 'urgent' ? 'selected' : '' }}>Urgent</option>
                                </select>
                            </div>
                            <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="surgeon_id" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500">
                                    ...
                                    @foreach($surgeons as $id => $name)
                                        <option value="{{ $id }}" {{ old('surgeon_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="anesthetist_id" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500">
                                    ...
                                    @foreach($anesthetists as $id => $name)
                                        <option value="{{ $id }}" {{ old('anesthetist_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="anesthesia_type" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500">
                                    ...
                                    <option value="regional" {{ old('anesthesia_type') == 'regional' ? 'selected' : '' }}>Regional</option>
                            ...
                            <div>
                                ...
                                <select name="assistant_doctor_id" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500">
                                    ...
                                    @foreach($surgeons as $id => $name)
                                        <option value="{{ $id }}" {{ old('assistant_doctor_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="nurse_id" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500">
                                    ...
                                    @foreach($nurses as $id => $name)
                                        <option value="{{ $id }}" {{ old('nurse_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div>
                                ...
                                <select name="ot_room_id" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500">
                                    ...
                                    @foreach($rooms as $id => $name)
                                        <option value="{{ $id }}" {{ old('ot_room_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <input type="date" name="scheduled_date" value="{{ old('scheduled_date', now()->format('Y-m-d')) }}" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500">
                            </div>
                            <div>
                                ...
                                <input type="time" name="scheduled_start" value="{{ old('scheduled_start', '08:00') }}" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">End Time (Estimated)</label>
                                <input type="time" name="scheduled_end" value="{{ old('scheduled_end') }}" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Estimated Cost ($)</label>
                                <input type="number" name="estimated_cost" value="{{ old('estimated_cost') }}" step="0.01" min="0" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500">
                            </div>
                            <div class="flex items-center gap-3">
                                <input type="checkbox" name="consent_signed" value="1" {{ old('consent_signed') ? 'checked' : '' }} class="rounded border-gray-300 text-purple-600 focus:ring-purple-500">
                    ...
                    <!-- Pre-Op Notes -->
                    <div class="space-y-4">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white border-b pb-2">Pre-Operative Notes</h3>
                        <textarea name="pre_op_notes" rows="3" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-purple-500" placeholder="Pre-operative instructions, patient preparation notes...">{{ old('pre_op_notes') }}</textarea>
                    </div>
                    ...
                    <div class="flex gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <a href="{{ route('hms.ot.index') }}" class="px-6 py-3 bg-gray-600 hover:bg-gray-700 text-white font-semibold rounded-lg shadow-md transition">
