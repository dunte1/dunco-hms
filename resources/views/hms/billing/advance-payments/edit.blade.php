    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-6">
                ...
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white"><i class="fa fa-edit text-emerald-600 mr-3"></i>Edit Advance Payment</h1>
            </div>

            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                <form action="{{ route('hms.advance-payments.update', $advancePayment) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="patient_id" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                                ...
                                @foreach($patients as $patient)
                                    <option value="{{ $patient->id }}" {{ old('patient_id', $advancePayment->patient_id) == $patient->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="amount" step="0.01" min="0.01" required value="{{ old('amount', $advancePayment->amount) }}"
                        ...
                        <div>
                            ...
                            <select name="status" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                                <option value="active" {{ old('status', $advancePayment->status) === 'active' ? 'selected' : '' }}>Active</option>
                                <option value="refunded" {{ old('status', $advancePayment->status) === 'refunded' ? 'selected' : '' }}>Refunded</option>
                                <option value="expired" {{ old('status', $advancePayment->status) === 'expired' ? 'selected' : '' }}>Expired</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date" name="payment_date" required value="{{ old('payment_date', $advancePayment->payment_date?->format('Y-m-d')) }}"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="notes" rows="3"
                                class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">{{ old('notes', $advancePayment->notes) }}</textarea>
