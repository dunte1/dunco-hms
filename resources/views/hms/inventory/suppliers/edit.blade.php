    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Edit Supplier') }}: {{ $supplier->display_name }}
            </h2>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('hms.inventory.suppliers.update', $supplier) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="supplier_type" required
                                    ...
                                    <option value="medicines" {{ old('supplier_type', $supplier->supplier_type) == 'medicines' ? 'selected' : '' }}>Medicines</option>
                                    <option value="equipment" {{ old('supplier_type', $supplier->supplier_type) == 'equipment' ? 'selected' : '' }}>Equipment</option>
                                    <option value="consumables" {{ old('supplier_type', $supplier->supplier_type) == 'consumables' ? 'selected' : '' }}>Consumables</option>
                                    <option value="food" {{ old('supplier_type', $supplier->supplier_type) == 'food' ? 'selected' : '' }}>Food</option>
                                    <option value="general" {{ old('supplier_type', $supplier->supplier_type) == 'general' ? 'selected' : '' }}>General</option>
                                </select>
                                @error('supplier_type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="payment_terms" required
                                    ...
                                    <option value="credit_15" {{ old('payment_terms', $supplier->payment_terms) == 'credit_15' ? 'selected' : '' }}>Credit 15 Days</option>
                                    <option value="credit_30" {{ old('payment_terms', $supplier->payment_terms) == 'credit_30' ? 'selected' : '' }}>Credit 30 Days</option>
                                    <option value="credit_60" {{ old('payment_terms', $supplier->payment_terms) == 'credit_60' ? 'selected' : '' }}>Credit 60 Days</option>
                                    <option value="credit_90" {{ old('payment_terms', $supplier->payment_terms) == 'credit_90' ? 'selected' : '' }}>Credit 90 Days</option>
                                </select>
                                @error('payment_terms')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="address" rows="3" required
                                          class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 @error('address') border-red-500 @enderror">{{ old('address', $supplier->address) }}</textarea>
                                @error('address')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="credit_limit" value="{{ old('credit_limit', $supplier->credit_limit) }}" min="0" step="0.01"
                                       class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Outstanding Balance (KES)</label>
                                <input type="number" name="outstanding_balance" value="{{ old('outstanding_balance', $supplier->outstanding_balance) }}" min="0" step="0.01"
                                       class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="status" required
                                    ...
                                    <option value="blocked" {{ old('status', $supplier->status) == 'blocked' ? 'selected' : '' }}>Blocked</option>
                                </select>
                                @error('status')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="notes" rows="3"
                                          class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">{{ old('notes', $supplier->notes) }}</textarea>
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="bank_branch" value="{{ old('bank_branch', $supplier->bank_branch) }}"
                                       class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
                            </div>
                        </div>
