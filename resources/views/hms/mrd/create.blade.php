    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-6">
                ...
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white"><i class="fa fa-plus-circle text-amber-600 mr-3"></i>Create MRD File</h1>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
                <div class="bg-gradient-to-r from-amber-500 to-amber-600 h-2"></div>
                <form method="POST" action="{{ route('hms.mrd.store') }}" class="p-6 space-y-4">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">File Type <span class="text-red-500">*</span></label>
                            <select name="file_type" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                                <option value="outpatient" {{ old('file_type') === 'outpatient' ? 'selected' : '' }}>Outpatient</option>
                                <option value="inpatient" {{ old('file_type') === 'inpatient' ? 'selected' : '' }}>Inpatient</option>
                                <option value="emergency" {{ old('file_type') === 'emergency' ? 'selected' : '' }}>Emergency</option>
                                <option value="maternity" {{ old('file_type') === 'maternity' ? 'selected' : '' }}>Maternity</option>
                                <option value="pediatric" {{ old('file_type') === 'pediatric' ? 'selected' : '' }}>Pediatric</option>
                            </select>
                        ...
                        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Physical Location</label>
                            <input type="text" name="physical_location" value="{{ old('physical_location') }}" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white" placeholder="e.g., Shelf A3, Room 102">
                            @error('physical_location')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror</div>
                        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Notes</label>
