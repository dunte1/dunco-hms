...
<div class="bg-white p-6 rounded shadow">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-semibold text-gray-800">Edit Currency: {{ $currency->name }}</h2>
    ...
    <form action="{{ route('admin.modules.multi-currency.update', $currency) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select id="decimal_places" 
                        ...
                        class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        required>
                    <option value="0" {{ old('decimal_places', $currency->decimal_places) == 0 ? 'selected' : '' }}>0 (Whole numbers)</option>
                    <option value="1" {{ old('decimal_places', $currency->decimal_places) == 1 ? 'selected' : '' }}>1</option>
                    <option value="2" {{ old('decimal_places', $currency->decimal_places) == 2 ? 'selected' : '' }}>2 (Standard)</option>
                    ...
                    <option value="5" {{ old('decimal_places', $currency->decimal_places) == 5 ? 'selected' : '' }}>5</option>
                    <option value="6" {{ old('decimal_places', $currency->decimal_places) == 6 ? 'selected' : '' }}>6</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select id="position" 
                        ...
                        class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        required>
                    <option value="before" {{ old('position', $currency->position) == 'before' ? 'selected' : '' }}>Before amount ($100)</option>
                    <option value="after" {{ old('position', $currency->position) == 'after' ? 'selected' : '' }}>After amount (100 $)</option>
                </select>
            </div>
        </div>
        ...
        <div>
            ...
            <textarea id="description" 
                      ...
                      rows="3"
                      class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                      placeholder="Optional description about this currency">{{ old('description', $currency->description) }}</textarea>
        </div>
